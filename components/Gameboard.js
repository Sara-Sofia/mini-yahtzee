...
export default Gameboard = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        calculateScore();
        if (gameEndStatus && totalThrows === 18 && allSpotsChosen) {
            endGame(playerName);
        }
    }, [numberOfThrowsLeft, totalThrows, allSpotsChosen, gameEndStatus]);


    // Calculating score, how far player is from bonuspoints and add bonuspoints to total score.
    ...
    // Resetting the game for the new game.
    const resetGame = () => {
    ...
    if (totalThrows === 0) {
        // If dices haven't been thrown, render this icon
        ...
        // If dices have been thrown, render the dices
